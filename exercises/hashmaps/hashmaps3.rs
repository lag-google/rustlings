...
fn build_scores_table(results: String) -> HashMap<String, Team> {
    ...
    for r in results.lines() {
        ...
        if ! scores.contains_key(&team_1_name) {
            scores.insert(team_1_name.to_string(), Team{name: team_1_name.to_string(), goals_scored: 0, goals_conceded: 0});
        }

        if ! scores.contains_key(&team_2_name) {
            scores.insert(team_2_name.to_string(), Team{name: team_2_name.to_string(), goals_scored: 0, goals_conceded: 0});
        }
